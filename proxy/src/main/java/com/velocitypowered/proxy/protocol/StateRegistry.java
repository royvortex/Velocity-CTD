  ...
  PLAY {
    {
      ...
      // CTD patch: defer ObjectivePacket and TeamPacket registration entirely to
      // VelocityScoreboardAPI plugin. The fork pre-registered these for 1.8+ client
      // support, but VSAPI now owns the full ObjectivePacket/TeamPacket mapping for
      // every protocol version (1.7.2 -> 26.1). Keeping these would cause
      // "Can not register class ObjectivePacket ... already registered" on every
      // version VSAPI tries to claim (1.9, 1.12, 1.13, etc).
      // Vanilla Velocity has no scoreboard API at all, and no other plugin needs
      // these mappings without VSAPI, so removal is safe when VSAPI is present.
      /*
      clientbound.register(
